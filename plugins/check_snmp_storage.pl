...
for ($i = 0; $i < $num_int; $i++) {
    ...
    if (defined($o_writable)) {
        $crit_state = !$fsaccess_table->{ $tindex[$i] };
    ...
    if (defined($o_writable) && ($fsaccess_table->{ $tindex[$i] })) {
        $output =~ s/\)(\s+)$/,RW)$1/;
    } elsif (defined($o_writable) && !($fsaccess_table->{ $tindex[$i] })) {
        $output =~ s/\)(\s+)$/,RO!)$1/;
    }
    ...
    $perf_out
        ...
        . round($to,     0);
    if (defined($o_writable)) {
        $perf_out .= ($fsaccess_table->{ $tindex[$i] } ? ";+W" : ";-W");
    }
}
